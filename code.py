def plot_probability_density_trio(grid, param_x: str = "slwp", param_y: str = "temperature_slwc",
                                   ...
                                   log_density=False,
                                   mask_type: str = "all",
                                   elev_threshold_low: float = 1000,   # seuil bas
                                   elev_threshold_high: float = 3000,  # seuil haut
                                   xlabel=None, ylabel=None):
    """
    Trace 3 PD côte à côte :
        (a) tout (pas de filtre élévation)
        (b) surface_elevation < elev_threshold_low
        (c) surface_elevation >= elev_threshold_high
    """
    ...
    masks = [
        ...
        (base_valid & (elev < elev_threshold_low),
         f"Elevation < {elev_threshold_low} m"),
        (base_valid & (elev >= elev_threshold_high),
         f"Elevation ≥ {elev_threshold_high} m"),
    ]
...
# Terre, seuil bas=500m, seuil haut=3000m
plot_probability_density_trio(grid, mask_type="land",
                               elev_threshold_low=500,
                               elev_threshold_high=3000,
                               d1="2025-12-01", d2="2025-12-31")

# Océan, seuil bas=0m, seuil haut=1000m
plot_probability_density_trio(grid, mask_type="ocean",
                               elev_threshold_low=0,
                               elev_threshold_high=1000,
                               d1="2025-12-01", d2="2025-12-31")